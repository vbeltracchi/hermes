...
def db_connect():
	...
	try:
		...
		if cnx.is_connected():
                	l.write(t + ' - mostfreq.py - Connected to Snort DB\n')
			l.close()
			return cnx
	except Error as e:
        	l.write(t + ' ' + e + '\n')
...
def main(argv):
	...
	query = ("select signature,count(*) as count,sig_name from event,signature where event.signature=signature.sig_id group by sig_name order by count desc;")
	# Execute the query
	cur.execute(query)
	...
	json_file = '/opt/stack/h-json/mostfreq.json'
