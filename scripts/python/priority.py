...
def db_connect():
	...
	try:
		...
		if cnx.is_connected():
                	l.write(t + ' - priority.py - Connected to Snort DB\n')
			l.close()
			return cnx
	except Error as e:
        	l.write(t + ' ' + e + '\n')
...
def main(argv):
	...
	query = ("select cid, cast(timestamp as char) as timestamp, sig_priority from event, signature where (event.signature)=(signature.sig_id) and (sig_priority>4) order by event.timestamp desc limit 10")
	# Execute the query
	cur.execute(query)
	...
	json_file = '/opt/stack/h-json/priority.json'
